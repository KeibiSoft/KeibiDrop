...
#
# Record the two-agent demo and put the KeibiDrop wordmark in the corner.
#
#   KDMCP=/path/to/kdmcp ./make-gif.sh
#
...
echo "Recording..."
( cd "$here" && KDMCP="${KDMCP:-$repo/kdmcp}" vhs demo.tape )
...
tmux kill-session -t kd-two-agents 2>/dev/null || true
pkill -f kdmcp 2>/dev/null || true
