...
func KD_Initialize(relayURL *C.char, inbound, outbound C.int, toMount, toSave *C.char) C.int {
	...
	instance, err := common.NewKeibiDrop(ctx, logger, false, parsed, int(inbound), int(outbound), m, s)
	if err != nil {
		return -2
	}
	kd = instance
