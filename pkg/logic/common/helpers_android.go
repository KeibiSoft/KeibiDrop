...
// GetLocalAddrs returns private/link-local IPs for LAN discovery.
// On Android, uses /proc/net/if_inet6 for IPv6 and a dial probe for IPv4.
func GetLocalAddrs() []string {
	...
	if err == nil {
		for _, e := range entries {
			if e.ip.IsLinkLocalUnicast() && e.iface != "lo" && e.iface != "dummy0" {
				addrs = append(addrs, e.ip.String()+"%"+e.iface)
			}
		}
	}
	conn, err := net.Dial("udp4", "10.255.255.1:1")
	if err == nil {
		defer conn.Close()
		host, _, err := net.SplitHostPort(conn.LocalAddr().String())
		if err == nil {
