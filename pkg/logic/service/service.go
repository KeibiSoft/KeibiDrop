...
// resolveBatchPath resolves one announced path to its real path, in FUSE or
// no-FUSE mode. Empty means not found.
func (kd *KeibidropServiceImpl) resolveBatchPath(path string) string {
	if (kd.FS() == nil || kd.FS().Root() == nil) && kd.SyncTracker != nil {
		lookupPath := strings.TrimPrefix(path, "/")
		kd.SyncTracker.LocalFilesMu.RLock()
		defer kd.SyncTracker.LocalFilesMu.RUnlock()
		if f, ok := kd.SyncTracker.LocalFiles[lookupPath]; ok {
			return f.RealPathOfFile
		}
		if f, ok := kd.SyncTracker.LocalFiles[path]; ok {
			return f.RealPathOfFile
		}
		return ""
	}
	if kd.FS() != nil && kd.FS().Root() != nil {
		return kd.resolveFUSEPath(path)
	}
	return ""
}

// ReadBatch streams a set of files in requested order. One request replaces a
// per-file round trip for each small file. The budget is soft: the current
// file finishes, no new file starts past it. A per-file failure sends an
// err_code frame and the batch continues.
func (kd *KeibidropServiceImpl) ReadBatch(req *bindings.ReadBatchRequest, stream bindings.KeibiService_ReadBatchServer) error {
	...
	for _, p := range req.Paths {
		...
		realPath := kd.resolveBatchPath(p)
		if realPath == "" {
